...
dfClean    = cleanData  (dfOriginal, Region, State)       # Collapse all states/counties into single row  
dfDaily    = createDaily(dfClean)                         # Create state/countries & values
dfRange    = getRange(dfDaily, StartDate = "2020-1-1",    # Limit date range
                               EndDate   = "2020-12-31")  

#
# Set a County, then plot it
#
County = "Sandoval"
RetVal = plotState(dfRange, County) # Plot it
